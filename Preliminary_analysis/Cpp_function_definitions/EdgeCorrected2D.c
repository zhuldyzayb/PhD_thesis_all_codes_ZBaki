...
void secondOpt(int *nl, int *np, double *sp1, double *sp2, double *gp1, double *gp2, 
               double *gridSize, double *hGlob, double *power, double *h, double *Measure, double *ans){
    double hA=*h;
    double M=*Measure;
    int nLocs=*nl;
    int nPoints=*np;
    ...
    for(i=0; i<nLocs; i++){
        c[i]=pow(lambdaCorr(nLocs,nPoints, sp1, sp2, sp1[i], sp2[i], gp1, gp2, hG, gridSize)/CC, -0.5);
    }   
    double val=0;
    double hh=0;
    for(i=0; i<nLocs; i++){
        hh=c[i]*hA;
        val+=1/lambda(nLocs, sp1, sp2, sp1[i], sp2[i], hh);
    }
